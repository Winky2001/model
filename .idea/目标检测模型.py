...
from utils import * #utils 工具类，在Python 开发中，将常用的功能封装成为接口，并放入Utils 工具类中，直接调用，可以提升效率
import torch.nn.functional as F#构建网络模型用到的包，可以通过调用函数的方式，直接来搭建网络，不需要先创对象
from math import sqrt #从 math 即数学库中导入用于开根运算的方法sqrt
from itertools import product as product
import torchvision #torchvision 是独立于pytorch 的关于图像操作的一些方便工具库
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")#挑选设备    torch.cuda.is_available() ：确定GPU是否可获得   torch.device()：选择设备
class VGGBase(nn.Module):#VGG基卷积产生底层特征图
    def __init__(self):
        super(VGGBase, self).__init__()#super类的作用是继承时，调用含super的各个基类__init__函数，使用super可以避免基类被重复调用
        # Standard convolutional layers in VGG16  标准卷积层
        self.conv1_1 = nn.Conv2d(3, 64, kernel_size=3, padding=1)  # stride = 1, by default
        self.conv1_2 = nn.Conv2d(64, 64, kernel_size=3, padding=1)#kernel_size为filters_size
        ...
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)  # ceiling (not floor) here for even dims     ceil_mode=True如果ceil_mode的值为True，那么则用天花板模式，否则用地板模式。
        ...
        self.conv6 = nn.Conv2d(512, 1024, kernel_size=3, padding=6, dilation=6)  # atrous convolution 深层卷积  dilation是对 kernel 进行膨胀，多出来的空隙用 0 padding。用于克服 stride 中造成的 失真问题。
     ...
     def forward(self, image):
         out = F.relu(self.conv1_1(image))  # (N, 64, 300, 300)  relu非线性激活函数
    ...
    def load_pretrained_layers(self):#负载预训练层
        ...
        class AuxiliaryConvolutions(nn.Module):辅助卷积
            def __init__(self):
            ...
            class PredictionConvolutions(nn.Module):#预期卷积
            ...
            def init_conv2d(self):
                        ...
                        for i in range(batch_size):
                            ...
                            decoded_locs = cxcy_to_xy(
                                gcxgcy_to_cxcy(predicted_locs[i],
                                               self.priors_cxcy))  # (8732, 4), these are fractional pt. coordinates
                            ...
                            for c in range(1, self.n_classes):
                                ...
                                class_scores, sort_ind = class_scores.sort(dim=0,
                                                                           descending=True)  # (n_qualified), (n_min_score)
                ...
                class MultiBoxLoss(nn.Module):
                    ...
                    def forward(self, predicted_locs, predicted_scores, boxes, labels):
                        ...
                        conf_loss = (
                                    conf_loss_hard_neg.sum() + conf_loss_pos.sum()) / n_positives.sum().float()  # (), scalar
